...
function Download-Ytplugin ($plugin, $version) {
    ...
    aria2c -x 16 -s 16 -j 16 -k 10M -o $plugin_exe $link
}
...
function Upgrade-Ytplugin {
    ...
    $ytdl = Check-GetYTDL
    if ($ytdl -eq 'ytdlp') {
        Write-Host "Remove old yt-dlp" -ForegroundColor Green
        Remove-Item -Path "D:\mpv\yt-dlp.exe" -Recurse -Force
        $latest_release = Get-Latest-Ytplugin "yt-dlp"
        Download-Ytplugin "yt-dlp" $latest_release
    }
    elseif ($ytdl -eq 'youtubedl') {
        Write-Host "Remove old youtube-dl" -ForegroundColor Green
        Remove-Item -Path "D:\mpv\youtube-dl.exe" -Recurse -Force
