...
var ucjsMouseGestures = {
  ...
  commands: [
    ...
    [
      "",
      "選択範囲のテキストリンクをすべてタブに開く(リンクが無い場合は規定の検索エンジンで検索)",
      function () {
  ...
  set statusinfo(val) {
    ...
    if (this._statusinfotimer) clearTimeout(this._statusinfotimer);
    this._statusinfotimer = setTimeout(() => {
      this.hideStatusInfo();
    }, this.STATUSINFO_TIMEOUT);
  ...
  init: function () {
    ...
    if (this.enableWheelGestures) window.addEventListener("wheel", this, true);

    messageManager.addMessageListener(
      "ucjsMouseGestures_linkURL_isWheelCancel",
      this,
    );
    ...
    messageManager.addMessageListener(
      "ucjsMouseGestures_linkURL_dragstart",
      this,
    );

    messageManager.addMessageListener(
      "ucjsMouseGestures_executeInChrome",
      this,
    );
  ...
  uninit: function () {
    ...
    messageManager.removeMessageListener(
      "ucjsMouseGestures_linkURL_start",
      this,
    );
    messageManager.removeMessageListener(
      "ucjsMouseGestures_linkURLs_stop",
      this,
    );
    messageManager.removeMessageListener(
      "ucjsMouseGestures_linkURL_dragstart",
  ...
  hideStatusInfo: function () {
    ...
    if (this._laststatusinfo == this.statusinfo) this.statusinfo = "";
  },
  ...
  receiveMessage: function (message) {
    ...
    switch (message.name) {
      ...
      case "ucjsMouseGestures_linkURL_start":
        ...
        this._linkReferrerInfo = E10SUtils.deserializeReferrerInfo(
          message.data.linkReferrerInfo,
        );
        ...
        this._cookieJarSettings = E10SUtils.deserializeCookieJarSettings(
          message.data.cookieJarSettings,
        );
        this._referrerInfo = E10SUtils.deserializeReferrerInfo(
      ...
      case "ucjsMouseGestures_linkURLs_stop":
        ...
        for (let i = 0; i < this._linkReferrerInfos.length; i++) {
          this._linkReferrerInfos[i] = E10SUtils.deserializeReferrerInfo(
        ...
        this._selLinkReferrerInfos =
          message.data.selLinkReferrerInfos.split(" ");
        for (let i = 0; i < this._selLinkReferrerInfos.length; i++) {
          this._selLinkReferrerInfos[i] = E10SUtils.deserializeReferrerInfo(
            this._selLinkReferrerInfos[i],
          );
        }
        break;
      case "ucjsMouseGestures_linkURL_dragstart":
        if (this.enableRockerGestures) this._isMouseDownL = false;
        break;
    ...
    switch (event.type) {
      ...
      case "mouseup":
        gBrowser.selectedBrowser.messageManager.sendAsyncMessage(
          "ucjsMouseGestures_mouseup",
        );
        gBrowser.tabpanels.removeEventListener("mousemove", this, false);
        if (
          (this._isMouseDownR && event.button == 2) ||
          (this._isMouseDownR &&
            this._isMac &&
            event.button == 0 &&
            event.ctrlKey)
          ...
          this._isMouseDownR = false;
          if (this._directionChain) this._suppressContext = true;
          this._stopGesture(event);
  ...
  _progressGesture: function (event) {
    ...
    var direction;
    if (distanceX > distanceY) direction = x < this._lastX ? "L" : "R";
    else direction = y < this._lastY ? "U" : "D";
    // compare to last direction
    var lastDirection = this._directionChain.charAt(
      this._directionChain.length - 1,
    );
    if (direction != lastDirection) {
      ...
      for (let command of this.commands) {
        if (command[0].substring(0, 1) == "*") {
          ...
          if (
            cmd ==
            this._directionChain.substring(
              this._directionChain.length - cmd.length,
            )
          ) {
  ...
  _stopGesture: function (event) {
    ...
    gBrowser.selectedBrowser.messageManager.sendAsyncMessage(
      "ucjsMouseGestures_linkURLs_request",
    );
        ...
        if (
          ...
          try {
            ...
            Services.console.logStringMessage(
              "Error in command (" + this._directionChain + ")" /*+ ex*/,
            );
          }
          this._directionChain = "";
          return;
  ...
  init: function () {
    let framescript = {
      ...
      init: function (isMac, enableWheelGestures) {
        ...
        this.console = Cc["@mozilla.org/consoleservice;1"].getService(
        ...
        if (this.enableWheelGestures) addEventListener("wheel", this, true);
      },
        ...
        switch (message.name) {
          ...
          case "ucjsMouseGestures_linkURLs_request":
            let [
              ...
              selLinkURLs,
              selLinkdocURLs,
              selLinkReferrerInfos,
            ] = this.gatherLinkURLsInSelection();
            let json = {
            ...
            sendSyncMessage("ucjsMouseGestures_linkURLs_stop", json);
            this.clearStyle();
            break;
        ...
        switch (event.type) {
              ...
              (_) => {
                ...
                */
                ...
                json = {
                  ...
                  linkReferrerInfo:
                    this.E10SUtils.serializeReferrerInfo(linkReferrerInfo),
                };
                sendSyncMessage("ucjsMouseGestures_linkURL_start", json);
              },
              0,
          ...
          case "mousemove":
            ...
            if (linkURL && this._linkURLs.indexOf(linkURL) == -1) {
              ...
              let linkReferrerInfo = Cc[
                "@mozilla.org/referrer-info;1"
              ].createInstance(Ci.nsIReferrerInfo);
              linkReferrerInfo.initWithElement(node);
              this._linkReferrerInfos.push(
                this.E10SUtils.serializeReferrerInfo(linkReferrerInfo),
              );
      ...
      _getLinkURL: function (aNode) {
        ...
        while (aNode) {
          if (
            (aNode instanceof content.HTMLAnchorElement ||
              aNode instanceof content.HTMLAreaElement) &&
            aNode.href
          ) {
            this.link = aNode;
            return aNode;
        ...
        while (aNode) {
          if (aNode instanceof content.HTMLImageElement && aNode.src) {
            ...
            try {
              ...
              var props = imageCache.findEntryProperties(aURL, aDoc);
            } catch (e) {}
            if (props) {
              try {
      ...
      gatherLinkURLsInSelection: function () {
        ...
        for (var i = 0; i < sel.rangeCount; i++) {
          ...
          var treeWalker = fragment.ownerDocument.createTreeWalker(
            fragment,
      ...
      executeInChrome: function (func, args) {
        ...
        sendAsyncMessage("ucjsMouseGestures_executeInChrome", json);
      },
      ...
      dispatchKeyEvent: function (
        ...
        content.document.querySelector(targetSelector).dispatchEvent(
          new content.KeyboardEvent(type, {
            bubbles: bubbles,
    ...
    window.messageManager.loadFrameScript(
      ...
      true,
    ); // Set the second parameter, allowDelayedLoad, to true, to automatically load the desired frame script in newly created browsers/tabs (of possibly newly created windows) as well. the third parameter, allow global access.
    delete framescript;
  },
};
...
let ucjsMouseGestures_helper = {
  ...
  dispatchEvent: function (event) {
    gBrowser.selectedBrowser.messageManager.sendAsyncMessage(
      "ucjsMouseGestures_dispatchEvent",
      event,
    );
  },
  ...
  dispatchKeyEvent: function (
    ...
    let json = {
      ...
      type: type,
      bubbles: bubbles,
      cancelable: cancelable /*viewArg: viewArg, */,
      ctrlKey: ctrlKey,
      shiftKey: shiftKey,
      altKey: altKey,
      metaKey: metaKey,
    ...
    gBrowser.selectedBrowser.messageManager.sendAsyncMessage(
      "ucjsMouseGestures_dispatchKeyEvent",
      json,
    );
  },
  ...
  commandsPopop: function (screenX, screenY) {
    ...
    if (typeof screenX == "undefined") screenX = that._lastX;
    if (typeof screenY == "undefined") screenY = that._lastY;
    ...
    popup.setAttribute(
      "oncommand",
      "ucjsMouseGestures_helper.doCommand(event);",
    );
    ...
    let ratio = 1;
    let os = Cc["@mozilla.org/system-info;1"]
      .getService(Ci.nsIPropertyBag2)
      .getProperty("name");
    if (os == "Darwin") {
      ratio = popup.ownerDocument.defaultView
  ...
  closedTabsPopup: function (screenX, screenY) {
    ...
    function populatePopup(undoPopup) {
      ...
      m.setAttribute(
        "accesskey",
        "R" /*strings.getString("menuRestoreAllTabs.accesskey")*/,
      );
      m.addEventListener(
        "command",
        function () {
          for (let i = 0; i < undoItems.length; i++) undoCloseTab();
        },
        false,
      );
      ...
      for (let i = 0; i < undoItems.length; i++) {
        ...
        for (let j = entries.length - 1; j > -1; j--) {
          if (j != entries.length - 1) tooltiptext += "\n";
        ...
        if (i == 0) m.setAttribute("key", "key_undoCloseTab");
      ...
      m.addEventListener(
        "command",
        function () {
          let max_undo = Services.prefs.getIntPref(
          ...
          if (
            max_undo !=
            Services.prefs.getIntPref("browser.sessionstore.max_tabs_undo", 10)
          )
            Services.prefs.setIntPref(
              "browser.sessionstore.max_tabs_undo",
              max_undo,
  ...
  _undoCloseMiddleClick: function PHM__undoCloseMiddleClick(aEvent) {
    ...
    if (!aEvent.ctrlKey) aEvent.target.parentNode.hidePopup();
  },
  ...
  sessionHistoryPopup: function (screenX, screenY) {
    ...
    aParent.setAttribute(
      "oncommand",
      "BrowserCommands.gotoHistoryIndex(event); event.stopPropagation();",
    );
    ...
    let os = Cc["@mozilla.org/system-info;1"]
      ...
      .getProperty("name");
    if (os == "Darwin") {
      ratio = aParent.ownerDocument.defaultView
        .QueryInterface(Ci.nsIInterfaceRequestor)
        .getInterface(Ci.nsIDOMWindowUtils).screenPixelsPerCSSPixel;
    }
    ...
    function updateSessionHistory(sessionHistory, initial, ssInParent) {
      ...
      if (!initial) {
        if (count <= 1) {
          ...
          aParent.id != "backForwardMenu" &&
          !aParent.parentNode.open
        ) {
  ...
  _webSearchPopupBuild: async function (aText, screenX, screenY) {
    ...
    popup.setAttribute(
      "oncommand",
      "ucjsMouseGestures_helper._loadSearch(event);",
    );
    /*popup.setAttribute("onclick", "checkForMiddleClick(this, event);");*/
    ...
    for (let i = engines.length - 1; i >= 0; --i) {
      ...
      menuitem.setAttribute(
        "class",
        "menuitem-iconic searchbar-engine-menuitem menuitem-with-favicon",
      );
    ...
    if (!!this.text) {
      ...
      if (selectedText.length > 15) {
        ...
        if (truncChar >= 0xdc00 && truncChar <= 0xdfff) truncLength++;
  ...
  getImageInfo: function (aURL) {
    try {
      ...
      var props = imageCache.findEntryProperties(aURL);
    } catch (e) {}
  ...
  copyHoverLinks: function () {
    ...
    setTimeout(() => {
      ...
      if (that._linkURLs.length > 1) urls += newLine;
      let clipboard = Cc["@mozilla.org/widget/clipboardhelper;1"].getService(
        Ci.nsIClipboardHelper,
      );
  ...
  copySelectedLinks: function () {
    ...
    setTimeout(() => {
      ...
      if (that._selLinkURLs.length > 1) urls += newLine;
      let clipboard = Cc["@mozilla.org/widget/clipboardhelper;1"].getService(
        Ci.nsIClipboardHelper,
      );
      clipboard.copyString(urls);
  ...
  goNumericURL: function (aIncrement) {
    ...
    for (let i = 0; i < digit; i++) num = "0" + num;
  ...
  openURLsInSelection: function () {
    ...
    if (!sel) throw "No selection";
    let URLs = [];
    sel.split(/\s/).forEach((str) => {
      let arrUrl = str.match(
        /((((h?t)?tps?|h..ps?)(:\/\/))?[a-zA-Z0-9+$;?.%,!#~*/:@&=_-]+)/gi,
      );
      if (!arrUrl) return;
    ...
    if (URLs.length > 0) this.openURLs(URLs);
    else ucjsMouseGestures_helper._loadSearchWithDefaultEngine(sel, true);
  },
  ...
  openHoverLinksInTabs: function () {
    let that = ucjsMouseGestures;
    setTimeout(() => {
      ucjsMouseGestures_helper.openURLs(
        that._linkURLs,
        that._linkReferrerInfos,
      );
    }, 500);
  },
  ...
  closeMultipleTabs: function (aLeftRight) {
    ...
    let shouldPrompt = Services.prefs.getBoolPref(
  ...
  openLinkInContainerTab: function (
    url = "",
    inBackground = true,
    relatedToCurrent = true,
    screenX,
    ...
    popup.setAttribute(
      "oncommand",
      "ucjsMouseGestures_helper.openLinkInTabFromContainerMenu(event, '" +
        url +
        "', " +
        inBackground +
        ", " +
  ...
  openLinkInTabFromContainerMenu: function (
    event,
    url = "",
    inBackground = true,
    relatedToCurrent = true,
