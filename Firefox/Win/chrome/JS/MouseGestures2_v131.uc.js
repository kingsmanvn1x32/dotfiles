...
var ucjsMouseGestures = {
  ...
  commands :
      ...
      [
        ...
        ['L', 'Quay lại', function(){ document.getElementById("Browser:Back").doCommand(); } ],
['R', 'Tiến tới', function(){ document.getElementById("Browser:Forward").doCommand(); } ],
['', 'Trang chủ', function(){ BrowserCommands.home(); } ],

['', 'Hiện popup lịch sử tab', function(){ ucjsMouseGestures_helper.sessionHistoryPopup(); } ],
['', 'Quay lại đầu lịch sử', function(){ SessionStore.getSessionHistory(gBrowser.selectedTab, history => {gBrowser.gotoIndex(history.entries.length = 0)}); } ],
['', 'Tiến tới cuối lịch sử', function(){ SessionStore.getSessionHistory(gBrowser.selectedTab, history => {gBrowser.gotoIndex(history.entries.length - 1)}); } ],
['RLR', '>> Tua nhanh', function(){ ucjsNavigation?.fastNavigationBackForward(1); } ],
['LRL', '<< Tua lùi', function(){ ucjsNavigation?.fastNavigationBackForward(-1); } ],

['RULD', 'Di chuyển lên một cấp', function(){ ucjsMouseGestures_helper.goUpperLevel(); } ],
['ULDR', 'Tăng số và di chuyển', function(){ ucjsMouseGestures_helper.goNumericURL(+1); } ],
['DLUR', 'Giảm số và di chuyển', function(){ ucjsMouseGestures_helper.goNumericURL(-1); } ],
['RL', 'Di chuyển đến liên kết [Tiếp theo]', function(){ ucjsNavigation?.advancedNavigateLink(1); } ],
['LR', 'Di chuyển đến liên kết [Quay lại]', function(){ ucjsNavigation?.advancedNavigateLink(-1); } ],

['UD', 'Tải lại', function(){ document.getElementById("Browser:Reload").doCommand(); } ],
['UDU', 'Tải lại (bỏ qua bộ nhớ đệm)', function(){ document.getElementById("Browser:ReloadSkipCache").doCommand(); } ],
['', 'Tải lại tất cả tab', function(){ typeof gBrowser.reloadTabs == "function" ? gBrowser.reloadTabs(gBrowser.visibleTabs) : gBrowser.reloadAllTabs(); } ],
['', 'Dừng tải', function(){ document.getElementById("Browser:Stop").doCommand(); } ],

['', 'Mở liên kết trong tab container được chỉ định', function(){ ucjsMouseGestures_helper.openLinkInContainerTab(); } ],

['', 'Mở tất cả liên kết văn bản trong vùng chọn thành tab (nếu không có liên kết, tìm kiếm bằng công cụ tìm kiếm mặc định)', function(){ ucjsMouseGestures_helper.openURLsInSelection(); } ],
['*RDL', 'Mở tất cả liên kết đã chọn thành tab', function(){ ucjsMouseGestures_helper.openSelectedLinksInTabs(); } ],
['*RUL', 'Mở tất cả liên kết đã di qua thành tab', function(){ ucjsMouseGestures_helper.openHoverLinksInTabs(); } ],
['', 'Mở liên kết trong tab mới', function(){ ucjsMouseGestures_helper.openURLs([ucjsMouseGestures._linkURL]); } ],

['', 'Lưu liên kết đã chọn', function(){ ucjsMouseGestures_helper.saveSelectedLinks(); } ],
['', 'Lưu liên kết đã di qua', function(){ ucjsMouseGestures_helper.saveHoverLinks(); } ],

['', 'Sao chép', function(){ ucjsMouseGestures_helper.copyText(ucjsMouseGestures._selectedTXT); } ],
['', 'Sao chép liên kết đã di qua', function(){ ucjsMouseGestures_helper.copyHoverLinks(); } ],
['', 'Sao chép liên kết đã chọn', function(){ ucjsMouseGestures_helper.copySelectedLinks(); } ],
...
['W-', 'Tab trước', function(){ gBrowser.tabContainer.advanceSelectedTab(-1, true); } ],
['W+', 'Tab tiếp theo', function(){ gBrowser.tabContainer.advanceSelectedTab(+1, true); } ],
/*['UL', 'Tab trước', function(){ gBrowser.tabContainer.advanceSelectedTab(-1, true); } ],
['UR', 'Tab tiếp theo', function(){ gBrowser.tabContainer.advanceSelectedTab(+1, true); } ],*/
['ULR', 'Tab được chọn ngay trước đó', function(){ ucjsNavigation_tabFocusManager?.advancedFocusTab(-1); } ],
['URL', 'Quay lại một tab được chọn ngay trước đó', function(){ ucjsNavigation_tabFocusManager?.advancedFocusTab(1); } ],

['', 'Mở tab mới', function(){ document.getElementById("cmd_newNavigatorTab").doCommand(); } ],
['', 'Mở tab container mới', function(){ ucjsMouseGestures_helper.openLinkInContainerTab("about:blank", false, false); } ],
['', 'Chuyển đổi ghim tab',
...
['LD', 'Đóng tab', function(){ document.getElementById("cmd_close").doCommand(); } ],
['', 'Đóng tab (ngoại trừ tab đã ghim)', function(){ if (!gBrowser.selectedTab.pinned) { document.getElementById("cmd_close").doCommand(); } } ],

['', 'Đóng tất cả tab bên trái', function(){ ucjsMouseGestures_helper.closeMultipleTabs("left"); } ],
['', 'Đóng tất cả tab bên phải', function(){ ucjsMouseGestures_helper.closeMultipleTabs("right"); } ],
['', 'Đóng tất cả tab khác', function(){ gBrowser.removeAllTabsBut(gBrowser.selectedTab); } ],
['LU', 'Khôi phục tab đã đóng', function(){ document.getElementById("History:UndoCloseTab").doCommand(); } ],
/*['DRU', 'Khôi phục tab đã đóng (từng cái một)', function(){undoCloseTab(0); } ],*/
['', 'Hiện popup danh sách tab đã đóng', function(){ ucjsMouseGestures_helper.closedTabsPopup(); } ],
['', 'Đóng tất cả tab', function(){ var browser = gBrowser; var ctab = browser.addTrustedTab(BROWSER_NEW_TAB_URL, {skipAnimation: true,}); browser.removeAllTabsBut(ctab); } ],
['', 'Đóng cửa sổ', function(){ document.getElementById("cmd_closeWindow").doCommand(); } ],

['', 'Thu nhỏ', function(){ window.minimize(); } ],
['', 'Phóng to/Khôi phục kích thước', function(){ window.windowState == 1 ? window.restore() : window.maximize(); } ],
['LDRU', 'Toàn màn hình', function(){ document.getElementById("View:FullScreen").doCommand(); } ],

['RU', 'Cuộn lên đầu', function(){ goDoCommand("cmd_scrollTop"); } ],
['RD', 'Cuộn xuống cuối', function(){ goDoCommand("cmd_scrollBottom"); } ],
['U', 'Cuộn lên', function(){ goDoCommand("cmd_scrollPageUp"); } ],
['D', 'Cuộn xuống', function(){ goDoCommand("cmd_scrollPageDown"); } ],
/*
['W-', 'Thu nhỏ', function(){ ucjsMouseGestures_helper.zoomIn(); } ],
['W+', 'Phóng to', function(){ ucjsMouseGestures_helper.zoomOut(); } ],*/
['L<R', 'Đặt lại thu phóng', function(){ ucjsMouseGestures_helper.zoomReset(); } ],

['DL', 'Thanh tìm kiếm trong trang',
  function(){
...
['DRD', 'Tìm kiếm với văn bản đã chọn (hiện popup công cụ tìm kiếm)', function(){ ucjsMouseGestures_helper.webSearchPopup(ucjsMouseGestures._selectedTXT || ucjsMouseGestures._linkTXT); } ],
['DR', 'Sao chép văn bản đã chọn vào thanh tìm kiếm',
  function(){ 
...
['', 'Chuyển đổi CSS', function(){ var styleDisabled = gPageStyleMenu._getStyleSheetInfo(gBrowser.selectedBrowser).authorStyleDisabled; if (styleDisabled) gPageStyleMenu.switchStyleSheet(""); else gPageStyleMenu.disableStyle(); } ],

['UDUD', 'Hiện popup lệnh cử chỉ', function(){ ucjsMouseGestures_helper.commandsPopop(); } ],
['', 'Khởi động lại', function(){ ucjsMouseGestures_helper.restart(); } ],

['', 'Thanh bên Dấu trang', function(){ SidebarController.toggle("viewBookmarksSidebar"); } ],
['', 'Thanh bên Lịch sử', function(){ SidebarController.toggle("viewHistorySidebar"); } ],

['', 'Xóa lịch sử gần đây', function(){ setTimeout(function(){ document.getElementById("Tools:Sanitize").doCommand(); }, 0); } ],
['', 'Bảng điều khiển trình duyệt', function(){ ucjsMouseGestures_helper.openBrowserConsole(); } ],
['', 'Quản lý tiện ích', function(){ openTrustedLinkIn("about:addons", "tab", {inBackground: false, relatedToCurrent: true}); } ],
['', 'Thông tin khắc phục sự cố', function(){ openTrustedLinkIn("about:support", "tab", {inBackground: false, relatedToCurrent: true}); } ],
['', 'Cài đặt (Tùy chọn)', function(){ openTrustedLinkIn("about:preferences", "tab", {inBackground: false, relatedToCurrent: true}); } ],
['', 'Chuyển đổi weAutopagerize',
  function(){
    ucjsMouseGestures_helper.dispatchEvent(
                    { target: "document", type: "AutoPagerizeToggleRequest" } );
  } ],
['', 'Chuyển đổi weAutopagerize phương pháp 2',
  function(){
      ...
      content.document.dispatchEvent(new content.Event("AutoPagerizeToggleRequest"));
        ...
        ['RDR', 'TWP - Switch between the translated and the original page「Ctrl+Alt+T」',
          function(){
            ...
            let keyEventT = new KeyboardEvent("", {key: "T", keyCode: KeyboardEvent.DOM_VK_T});
            let keyEventCtrl = new KeyboardEvent("", {key: "Control", keyCode: KeyboardEvent.DOM_VK_CONTROL});
            let keyEventAlt = new KeyboardEvent("", {key: "Alt", keyCode: KeyboardEvent.DOM_VK_ALT});
            tip.keydown(keyEventCtrl);
            tip.keydown(keyEventAlt);
            tip.keydown(keyEventT);
            tip.keyup(keyEventT);
            tip.keyup(keyEventAlt);
        ...
        ['LDR', 'TWP - Translate selected text「Ctrl+Alt+S」',
          function(){
            ...
            let keyEventS = new KeyboardEvent("", {key: "S", keyCode: KeyboardEvent.DOM_VK_S});
            let keyEventCtrl = new KeyboardEvent("", {key: "Control", keyCode: KeyboardEvent.DOM_VK_CONTROL});
            let keyEventAlt = new KeyboardEvent("", {key: "Alt", keyCode: KeyboardEvent.DOM_VK_ALT});
            tip.keydown(keyEventCtrl);
            tip.keydown(keyEventAlt);
            tip.keydown(keyEventS);
            tip.keyup(keyEventS);
            tip.keyup(keyEventAlt);
            tip.keyup(keyEventCtrl);
          } ],
        ...
        ['DLU', 'TWP - Translate the page to the target language 1「Shift+Alt+1」',
          function(){
            ...
            let keyEvent1 = new KeyboardEvent("", {key: "1", keyCode: KeyboardEvent.DOM_VK_1});
            let keyEventShift = new KeyboardEvent("", {key: "Shift", keyCode: KeyboardEvent.DOM_VK_SHIFT});
            let keyEventAlt = new KeyboardEvent("", {key: "Alt", keyCode: KeyboardEvent.DOM_VK_ALT});
            tip.keydown(keyEventShift);
            tip.keydown(keyEventAlt);
            tip.keydown(keyEvent1);
            tip.keyup(keyEvent1);
            tip.keyup(keyEventAlt);
            tip.keyup(keyEventShift);
          } ],
        ...
        ['RULDR', 'TWP - Translate the page to the target language 2「Shift+Alt+2」',
          function(){
            ...
            let keyEvent2 = new KeyboardEvent("", {key: "2", keyCode: KeyboardEvent.DOM_VK_2});
            let keyEventShift = new KeyboardEvent("", {key: "Shift", keyCode: KeyboardEvent.DOM_VK_SHIFT});
            let keyEventAlt = new KeyboardEvent("", {key: "Alt", keyCode: KeyboardEvent.DOM_VK_ALT});
            tip.keydown(keyEventShift);
            tip.keydown(keyEventAlt);
            tip.keydown(keyEvent2);
            tip.keyup(keyEvent2);
            tip.keyup(keyEventAlt);
            tip.keyup(keyEventShift);
          } ],

        ['', 'Lưu tất cả các Canvas trên trang',
          function() {
